...
$(DIST)/$(PACKAGE)_$(VERSION)_$(ARCH).deb: $(TARGET)/$(PACKAGE)_$(VERSION)_$(ARCH)
	...
	find $(TARGET)/$(PACKAGE) -type f -exec sed -i '' "s/__PACKAGE__/$(PACKAGE)/g" {} \;
	find $(TARGET)/$(PACKAGE) -type f -exec sed -i '' "s/__VERSION__/$(VERSION)/g" {} \;
	find $(TARGET)/$(PACKAGE) -type f -exec sed -i '' "s/__DESCRIPTION__/$(DESCRIPTION)/g" {} \;
	find $(TARGET)/$(PACKAGE) -type f -exec sed -i '' "s/__ARCH__/$(ARCH)/g" {} \;
	find $(TARGET)/$(PACKAGE) -type f -exec sed -i '' "s/__MAINTAINER__/$(MAINTAINER)/g" {} \;
